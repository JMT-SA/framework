...
class FruitSizeReferenceInteractor < BaseInteractor
  def fruit_size_reference_repo
    @fruit_size_reference_repo ||= FruitSizeReferenceRepo.new
  end

  def fruit_size_reference(cached = true)
    if cached
      @fruit_size_reference ||= fruit_size_reference_repo.find(@id)
    else
      @fruit_size_reference = fruit_size_reference_repo.find(@id)
    end
  end
  ...
  def create_fruit_size_reference(parent_id, params)
    ...
    @id = fruit_size_reference_repo.create(res.to_h)
  ...
  def update_fruit_size_reference(id, params)
    ...
    res = validate_fruit_size_reference_params(params)
    return validation_failed_response(res) unless res.messages.empty?
    fruit_size_reference_repo.update(id, res.to_h)
  ...
  def delete_fruit_size_reference(id)
    ...
    fruit_size_reference_repo.delete(id)
