...
class TestCustomerRoutes < RouteTester
  ...
  def test_update
    ...
    ensure_exists!(INTERACTOR)
    row_vals = Hash.new(1)
    MasterfilesApp::CustomerInteractor.any_instance.stubs(:update_customer).returns(ok_response(instance: row_vals))
    patch 'masterfiles/parties/customers/1', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
  ...
  def test_update_fail
    authorise_pass!
    ensure_exists!(INTERACTOR)
    MasterfilesApp::CustomerInteractor.any_instance.stubs(:update_customer).returns(bad_response)
    Masterfiles::Parties::Customer::Edit.stub(:call, bland_page) do
      patch 'masterfiles/parties/customers/1', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
    end
  ...
  def test_delete
    authorise_pass!
    ensure_exists!(INTERACTOR)
    MasterfilesApp::CustomerInteractor.any_instance.stubs(:delete_customer).returns(ok_response)
    delete 'masterfiles/parties/customers/1', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
  ...
  #   authorise_pass!
  #   ensure_exists!(INTERACTOR)
  #   MasterfilesApp::CustomerInteractor.any_instance.stubs(:delete_customer).returns(bad_response)
  #   delete 'masterfiles/parties/customers/1', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
  #   expect_bad_redirect
  # end
  ...
  def test_create
    authorise_pass!
    ensure_exists!(INTERACTOR)
    MasterfilesApp::CustomerInteractor.any_instance.stubs(:create_customer).returns(ok_response)
  ...
  def test_create_fail
    authorise_pass!
    ensure_exists!(INTERACTOR)
    MasterfilesApp::CustomerInteractor.any_instance.stubs(:create_customer).returns(bad_response)
    Masterfiles::Parties::Customer::New.stub(:call, bland_page) do
      post_as_fetch 'masterfiles/parties/customers', { customer: { party_id: 1 } }, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
    end
    expect_bad_page
