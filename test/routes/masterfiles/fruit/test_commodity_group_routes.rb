...
class TestCommodityGroupRoutes < RouteTester
  ...
  def test_update
    ...
    ensure_exists!(INTERACTOR)
    row_vals = Hash.new(1)
    INTERACTOR.any_instance.stubs(:update_commodity_group).returns(ok_response(instance: row_vals))
  ...
  def test_update_fail
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:update_commodity_group).returns(bad_response)
    Masterfiles::Fruit::CommodityGroup::Edit.stub(:call, bland_page) do
      patch 'masterfiles/fruit/commodity_groups/1', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
    end
  ...
  def test_delete
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:delete_commodity_group).returns(ok_response)
  ...
  def test_create
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:create_commodity_group).returns(ok_response)
  ...
  def test_create_fail
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:create_commodity_group).returns(bad_response)
    Masterfiles::Fruit::CommodityGroup::New.stub(:call, bland_page) do
      post 'masterfiles/fruit/commodity_groups', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
    end
