    ...
    module StockMovementReport
      class Show
        def self.call(start_date, end_date, form_values: nil, form_errors: nil) # rubocop:disable Metrics/AbcSize
          ui_rule = UiRules::Compiler.new(:stock_movement_report, :new, form_values: form_values, start_date: start_date, end_date: end_date)
          ...
          layout = Crossbeams::Layout::Page.build(rules) do |page|
            ...
            page.section do |section|
              section.add_grid('stock_movement_report',
                               "/pack_material/transactions/movement_report/report?start_date=#{start_date}&end_date=#{end_date}",
                               caption: 'Stock Movement Report')
            end

            page.section do |section|
              section.add_grid('stock_movement_records',
                               "/pack_material/transactions/movement_report/records?start_date=#{start_date}&end_date=#{end_date}",
