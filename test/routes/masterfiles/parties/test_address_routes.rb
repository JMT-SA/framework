...
class TestAddressRoutes < RouteTester
  ...
  def test_update
    ...
    ensure_exists!(INTERACTOR)
    row_vals = Hash.new(1)
    INTERACTOR.any_instance.stubs(:update_address).returns(ok_response(instance: row_vals))
  ...
  def test_update_fail
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:update_address).returns(bad_response)
    Masterfiles::Parties::Address::Edit.stub(:call, bland_page) do
      patch_as_fetch 'masterfiles/parties/addresses/1', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
    end
  ...
  def test_delete
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:delete_address).returns(ok_response)
  ...
  def test_create
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:create_address).returns(ok_response)
  ...
  def test_create_fail
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:create_address).returns(bad_response)
    Masterfiles::Parties::Address::New.stub(:call, bland_page) do
      post 'masterfiles/parties/addresses', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
    end
    expect_flash_error
