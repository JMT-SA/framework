...
class InitStockJournalOpeningBalances < BaseScript
  def run # rubocop:disable Metrics/AbcSize
    ...
    dates = DB["select generate_series(date_trunc('month', date('#{AppConst::STOCK_TAKE_ON_DATE}'))::date, date_trunc('month', current_date)::date, interval '1 month');"].all.map { |r| r[:generate_series].strftime('%Y-%m-%d') }
    dates.each do |date|
      script = %(INSERT INTO stock_journal_entries (mr_product_variant_id, opening_balance, opening_balance_on)
                  SELECT mrpv.id, fn_stock_total_at(mrpv.id, date_trunc('month', date('#{AppConst::STOCK_TAKE_ON_DATE}'))::date, ('#{date}')::date), ('#{date}')::date
