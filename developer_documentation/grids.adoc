...
The grid will automatically set the font colour of a row to red if the row has a column named `active` which has a value of `false`.
...
----
SELECT
CASE WHEN cancelled THEN 'red b'  -- red and bold classes for cancelled
     WHEN approved THEN 'green'   -- green for approved
     WHEN completed THEN 'orange' -- orange for complete but not yet approved
...
Some useful Tachyons (and built-in background colour) classes:
