...
class TestFruitSizeReferenceRoutes < RouteTester
  ...
  def test_update
    ...
    ensure_exists!(INTERACTOR)
    row_vals = Hash.new(1)
    INTERACTOR.any_instance.stubs(:update_fruit_size_reference).returns(ok_response(instance: row_vals))
    patch_as_fetch 'masterfiles/fruit/fruit_size_references/1', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
  ...
  def test_update_fail
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:update_fruit_size_reference).returns(bad_response)
    Masterfiles::Fruit::FruitSizeReference::Edit.stub(:call, bland_page) do
      patch_as_fetch 'masterfiles/fruit/fruit_size_references/1', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
    end
  ...
  def test_create
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:create_fruit_size_reference).returns(ok_response)
  ...
  def test_create_fail
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:create_fruit_size_reference).returns(bad_response)
    Masterfiles::Fruit::FruitSizeReference::New.stub(:call, bland_page) do
      post 'masterfiles/fruit/fruit_actual_counts_for_packs/1/fruit_size_references', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
    end
