...
class Framework < Roda
  route 'config', 'pack_material' do |r|
    ...
    # --------------------------------------------------------------------------
    # Note: Material Resource Product Variants and Party Roles apply to other applications of the generic parts of this code
    # If we were to have a Manufacturing Product for example; it would also use the following code for mr_product_variants.
    # That is why this code was in a route file without a menu item.
      ...
      r.on 'material_resource_product_variant_party_roles' do
        ...
        r.post do        # CREATE
          ...
          if res.success
            ...
            re_show_form(r, res, url: "/pack_material/config/material_resource_product_variants/#{id}/material_resource_product_variant_party_roles/new?type=#{type}") do
    ...
    r.on 'link_alternative_material_resource_product_variants', Integer do |id|
      r.post do
        interactor = PackMaterialApp::MatresProductVariantInteractor.new(current_user, {}, { route_url: request.path }, {})
        store_last_referer_url(:link_alternative_material_resource_product_variants)
        ...
        redirect_to_stored_referer(r, :link_alternative_material_resource_product_variants)
      end
    end
    r.on 'link_co_use_material_resource_product_variants', Integer do |id|
      r.post do
        interactor = PackMaterialApp::MatresProductVariantInteractor.new(current_user, {}, { route_url: request.path }, {})
        store_last_referer_url(:link_co_use_material_resource_product_variants)
        ...
        redirect_to_stored_referer(r, :link_co_use_material_resource_product_variants)
      end
    end
  end
end
