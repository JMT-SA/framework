...
class TestContactMethodRoutes < RouteTester
  ...
  INTERACTOR = MasterfilesApp::ContactMethodInteractor
  ...
  def test_update
    ...
    ensure_exists!(INTERACTOR)
    row_vals = Hash.new(1)
    INTERACTOR.any_instance.stubs(:update_contact_method).returns(ok_response(instance: row_vals))
  ...
  def test_update_fail
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:update_contact_method).returns(bad_response)
    Masterfiles::Parties::ContactMethod::Edit.stub(:call, bland_page) do
      patch 'masterfiles/parties/contact_methods/1', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
    end
  ...
  def test_delete
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:delete_contact_method).returns(ok_response)
  ...
  def test_create
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:create_contact_method).returns(ok_response)
  ...
  def test_create_fail
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:create_contact_method).returns(bad_response)
    Masterfiles::Parties::ContactMethod::New.stub(:call, bland_page) do
      post 'masterfiles/parties/contact_methods', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
    end
    expect_flash_error
