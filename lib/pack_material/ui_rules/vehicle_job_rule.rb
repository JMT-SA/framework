...
module UiRules
  class VehicleJobRule < Base
    ...
    def edit_fields
      {
        ...
        vehicle_id: { renderer: :select,
                      options: @repo.for_select_vehicles,
                      caption: 'Vehicle',
                      required: true },
        departure_location_id: { renderer: :select,
                                 options: @tripsheets_repo.departure_locations,
                                 caption: 'Departure Location',
                                 required: true },
        tripsheet_number: { renderer: :label },
        planned_location_id: { renderer: :select,
                               options: receiving_bays,
                               selected: @form_object.planned_location_id,
                               caption: 'Planned Location To',
                               prompt: 'Leave blank if same Building' },
        virtual_location_id: { renderer: :hidden },
    ...
    def virtual_location_id
      @location_repo.for_select_locations(where: { virtual_location: true, can_store_stock: true, restricted: true }).first.last
    end
  end
end
