...
module DataminerApp
  class DataminerInteractor < BaseInteractor
    ...
    def prepared_report_grid(id)
      ...
      # row_defs = repo.db_connection_for(db)[sql_to_run(repo.db_connection_for(db), report)].to_a.map do |m|
      #   m.each_key { |k| m[k] = m[k].to_f if m[k].is_a?(BigDecimal) }
      #   m
      # end
      #
      # s = String.new("<table><tr><th>#{report.ordered_columns.map(&:caption).join('</th><th>')}</th></tr>")
      # row_defs.each do |record|
      #   s << '<tr>'
      #   report.ordered_columns.each do |k|
    ...
    def prepared_report_as_html(id)
      ...
      row_defs = repo.db_connection_for(db)[sql_to_run(repo.db_connection_for(db), rpt)].to_a.map do |m|
        m.each_key { |k| m[k] = m[k].to_f if m[k].is_a?(BigDecimal) }
        m
      end
