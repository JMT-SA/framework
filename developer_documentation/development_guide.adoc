...
Most classes in code should be tachyons classes (e.g. `class="b blue br3"` means `font-weight: bold`, `color: blue` `border-radius: .5rem`).
...
`checkp` - searches source code for debugging `p` or `puts` statements.

[source,bash]
alias checkp="ag '\s(p|puts)\s' lib helpers routes test"

`gitfiles` - lists files that have not been committed. Zip them with `gitfiles | zip afile.zip -@` or open all in vim with `vim $(gitfiles)`.
[source,bash]
