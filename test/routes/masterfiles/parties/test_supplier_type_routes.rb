...
class TestSupplierTypeRoutes < RouteTester
  ...
  def test_update
    ...
    ensure_exists!(INTERACTOR)
    row_vals = Hash.new(1)
    INTERACTOR.any_instance.stubs(:update_supplier_type).returns(ok_response(instance: row_vals))
  ...
  def test_update_fail
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:update_supplier_type).returns(bad_response)
    Masterfiles::Parties::SupplierType::Edit.stub(:call, bland_page) do
      patch 'masterfiles/parties/supplier_types/1', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
    end
  ...
  def test_delete
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:delete_supplier_type).returns(ok_response)
  ...
  def test_create
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:create_supplier_type).returns(ok_response)
  ...
  def test_create_fail
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:create_supplier_type).returns(bad_response)
    Masterfiles::Parties::SupplierType::New.stub(:call, bland_page) do
      post_as_fetch 'masterfiles/parties/supplier_types', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
    end
    expect_bad_page
